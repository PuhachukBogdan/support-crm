...
describe('Labels — scoping and access', () => {
  ...
  it('is NOT_FOUND for a conversation in a brand the caller may not serve — and writes nothing', async () => {
    const { prisma, conversationLabel } = fakePrisma({
      convFindFirst: jest.fn().mockResolvedValue(conversationRow({ brand_id: 'brand-z' })),
    });
    await expect(
      build(prisma).attachLabel({ conversationId: 'c1', labelId: 'l1' }, md('acc-1', ['brand-a'])),
    ).rejects.toBeInstanceOf(RpcException);
    expect(conversationLabel.upsert).not.toHaveBeenCalled();
  });
