#!/usr/bin/env bash
# Run the W15 browser check against the stand's PUBLIC origin — the Channels admin section.
#
...
#
# ⓘ Sibling of `run-w14-browser-check.sh` — same harness, next screen. `lib/` MUST be copied
# alongside (files are copied into /tmp/pw-work one by one).
set -euo pipefail
EDGE_PW=$(tr -d '\r\n' < /tmp/probe.pw)
OWNER_PW=$(tr -d '\r\n' < /tmp/owner.pw)
cp "$(dirname "$0")/w15-browser-check.mjs" /tmp/pw-work/w15-browser-check.mjs
...
docker run --rm --network host -v /tmp:/tmp -w /tmp/pw-work \
  ...
  mcr.microsoft.com/playwright:v1.50.0-noble node /tmp/pw-work/w15-browser-check.mjs
