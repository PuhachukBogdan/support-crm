...
// ⭐ Feature 037 (roadmap 4.15 — W30): the ticket-field taxonomy, transcribed from the operator's
// Zendesk screenshots. Its own file because it is 46 fields of DATA with one rule per line — and
// because the value lists have a source of truth outside this repo
// (`cowork/_archive/zendesk-capture-raw/ticket-field-values.md`) that a reviewer will diff against.
import { FIELD_SEED } from './seed.fields';
...
export function buildSeed() {
  const seed = {
    ...
    /**
     * ⭐ Feature 037 (roadmap 4.15 — W30): the team's OWN ticket-field taxonomy — 46 fields, 45
     * option sets and 11 forms, transcribed from the operator's Zendesk screenshots (12.08.2026)
     * and expanded in `seed.fields.ts`, which holds the values and the reasoning. ⚠️ It replaced
     * this block's first draft, whose lists were EXAMPLES invented from one frame — the real ones
     * arrived the same day and the invented placeholders (Brazil, Provider error, …) are gone.
     */
    ...FIELD_SEED,
    conversations: [
      {
