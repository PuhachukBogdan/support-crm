#!/usr/bin/env bash
# Run the W19 browser check against the stand's PUBLIC origin — my avatar, my status.
#
...
#
# ⓘ Sibling of `run-w18-browser-check.sh` — same harness, next screen. `lib/` MUST be copied
# alongside (files are copied into /tmp/pw-work one by one).
set -euo pipefail
EDGE_PW=$(tr -d '\r\n' < /tmp/probe.pw)
cp "$(dirname "$0")/w19-browser-check.mjs" /tmp/pw-work/w19-browser-check.mjs
...
docker run --rm --network host -v /tmp:/tmp -w /tmp/pw-work \
  ...
  mcr.microsoft.com/playwright:v1.50.0-noble node /tmp/pw-work/w19-browser-check.mjs
