 ...
 * coverage test (Permission / UserPermissionSet / AuditEntry are enrolled in SCOPED_MODELS);
...
describe('RBAC management is account-isolated (Principle I / SC-011)', () => {
  it('personalize for account A stamps only A; for B only B; nothing crosses', async () => {
    ...
    await overrides.personalizeUser('acct-A', { userId: 'god-a' }, 'u-a', 'reports.export', true);
    await overrides.personalizeUser('acct-B', { userId: 'god-b' }, 'u-b', 'reports.export', true);
    ...
    const auditsA = fake._tables.auditEntries.filter((a) => a.account_id === 'acct-A');
    const auditsB = fake._tables.auditEntries.filter((a) => a.account_id === 'acct-B');
    ...
    expect(
      fake._tables.auditEntries.every((a) => a.account_id === 'acct-A' || a.account_id === 'acct-B'),
    ).toBe(true);
  });
});
