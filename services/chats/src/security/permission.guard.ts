...
import { REQUIRED_CHATS_PERMISSION_KEY } from './requires-chats-permission.decorator';

/**
 * Service-tier RBAC guard for chats handlers (feature 012, mirrors the Users PlayerAccessGuard).
 * Enforces the required permission against the caller-permission CONTEXT the gateway sets in gRPC
 * metadata (`x-actor-permissions`, comma-joined). This is the SECOND, independent tier (Principle
 * II / SC-004): a call that skips the gateway carries no valid context → PERMISSION_DENIED. Chats
 * never reads auth_db (Principle VIII) — it trusts only the resolved context passed to it.
 *
 * Explicit @Inject: the service runtime (tsx/esbuild) emits no decorator metadata.
 */
@Injectable()
export class ChatsAccessGuard implements CanActivate {
